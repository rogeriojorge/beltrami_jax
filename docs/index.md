...
- SPECTRE-shaped `Ate/Aze/Ato/Azo` solves, derivative vector-potential blocks, current diagnostics, selected local constraint updates, and fixed-boundary/free-boundary `Lconstraint=3` global-current closure

It does not yet include all of SPEC or a full JAX-native SPECTRE Beltrami backend because the virtual-casing/free-boundary normal-field update, non-stellarator-symmetric diagnostics, broader fixtures, and production sparse scaling remain open.
