...
### SPECTRE local transform closure

The `G2V32L1Fi` case is the first fully JAX-native local rotational-transform closure test. It starts from the TOML initial `mu` and flux increments, assembles each volume in JAX, solves the Beltrami system, computes SPECTRE's straight-field-line transform diagnostic from the solved coefficients, and Newton-updates `mu`/`dpflux` until the `iota(lvol)` and `oita(lvol-1)` residuals close.

![SPECTRE Lconstraint=1 closure](./_static/spectre_lconstraint1_transform.png)

Committed validation summary:

- worst interface transform residual: `6.99e-15`
- worst post-constraint `mu`/`psi` state error versus SPECTRE: `3.18e-14`
- worst per-volume `Ate/Aze/Ato/Azo` relative coefficient error: `1.22e-14`
- generated by `tools/generate_spectre_lconstraint1_validation_assets.py`
